...
function Env(obj){
    ...
    if(obj.pars !== 0){
        for(var i = 0; i < obj.pars.length; i += 1){
            env[obj.pars[i]] = obj.args[i];
        }
    }
    env.find = find;
    return env;
...
function addGlobal(env){
    var names = ['abs', 'acos', 'asin', 'atan', 'atan2', 'ceil', 'cos', 'exp', 'floor', 'log', 'max', 'min', 'pow', 'random', 'round', 'sin', 'sqrt', 'tan'];

    for (var i = 0; i < names.length; i += 1) {
        env[names[i]] = Math[names[i]];
    }
...
function repl() {
    ...
    stdin.resume();
    stdout.write(">>> ");
    stdin.on('data',function(obja){
    obja = (obja + "").trim();
    var result = eval(read(obja));
    if (result != undefined)
        stdout.write(result+'\n>>> ');
    else
        stdout.write('>>> ');});
